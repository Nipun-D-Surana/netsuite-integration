    ...
    function execute(context) {
      try {
        ...
        if (searchResult.length > 0) {
            ...
            // Check Archive Export Syrup Integration CSV Folder is created or not 
            var archiveFolderId = search
              .create({
                type: search.Type.FOLDER,
                filters: [['name', 'is', 'Archive Export Syrup Integration CSV']],
                columns: ['internalid']
              })
              .run()
              .getRange({ start: 0, end: 1 })
            ...
            // Made Archive Export Syrup Integration CSV folder in NetSuite File Cabinet
            if (archiveFolderId == null) {
                var folder = record.create({ type: record.Type.FOLDER});
                folder.setValue({ fieldId: 'name',
                        value: 'Archive Export Syrup Integration CSV' });
                        archiveFolderId = folder.save();
                log.debug("Made Archive Export Syrup Integration CSV folder in NetSuite File Cabinet with Id ! " + archiveFolderId);
            }
            ...
            */
            ...
            sftpDirectory = sftpDirectory + 'SyrupIntegration';
            ...
            for (var i = 0; i < searchResult.length; i++) {
              try {
                ...
                log.debug("Export Syrup Integration CSV File Uploaded Successfully to SFTP server with file Id " + fileObj.id);
                ...
                log.error({
                  title: 'Error in uploading Export Syrup Integration CSV files',
                  details: e,
                });
              }
            }
        ...
        log.error({
          title: 'Error in processingExport Syrup Integration CSV files',
          details: e,
        });
        throw error.create({
          name:"Error in processing Export Syrup Integration CSV files",
          message: e
        });
      }
    }

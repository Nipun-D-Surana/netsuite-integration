      ...
      try {
        ...
        for (var i=0; i<list.length; i++) {
          ...
          try {
            ...
            scriptTask.mappingId = 'custimport_hc_import_syrup_to';
            scriptTask.importFile = downloadedFile;
            scriptTask.name = 'Add Syrup Transfer Order' + '-' + fileName;
            var csvImportTaskId = scriptTask.submit();
            
            var taskStatus = task.checkStatus(csvImportTaskId);
            if (taskStatus.status === 'FAILED') {
              log.debug("Import Syrup Transfer Order CSV task has been failed");
            } else {
              // Archive the file
              connection.move({
              ...
              log.error({
                title: 'Error in processing Syrup Transfer Order csv files',
                details: e,
              });
              ...
              if (errorList.length !== 0) {
                ...
                var date = new Date();
                var errorFileName = date + '-ErrorSyrupTransferOrder.csv';
        ...
        log.error({
          title: 'Error in importing Syrup Transfer Order csv files',
          details: e,
        });
        throw error.create({
          name:"Error in importing Syrup Transfer Order csv files",
          message: e
        });
      }
   }
